...
function formatHtml(frames) {
    ...
    frames.forEach(function (frame) {
        result += '<li>';
        result += '  <div class="frame">' + Utils.htmlEscape(frame.frame) + '</div>';
        if (frame.code && frame.code.length) {
            result += '  <ul class="source">';
            frame.code.forEach(function (line) {
                result += '  <li ' + (line.isErrorLine ? 'class="error-line"' : '') + '>';
                result += '    <pre>';
                result += Utils.padLeft(line.linenum.toString(), 4);
                result += ': ';
                result += Utils.htmlEscape(line.code);
                result += '</pre>';
                result += '  </li>';
            });
            result += '  </ul>';
        ...
        if (err instanceof Error) {
            ...
            FS.readFile(__dirname + '/../public/style.css', 'utf8', function (e, style) {
                FS.readFile(__dirname + '/../public/error.html', 'utf8', function (e, html) {
                    var stack = formatHtml(err.stack);
                    html = html
                        .replace('{style}', style)
                        .replace('{stack}', stack)
                        .replace('{title}', title)
                        .replace('{statusCode}', res.statusCode)
                        .replace(/\{error\}/g, Utils.htmlEscape(err.message));
